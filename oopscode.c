...
#define inct                                                                   \
  totalt++;                                                                    \
  t++; /*increment time and total time*/
#define tncheck                                                                \
  if (n < 0 || t + n > T * P)                                                  \
    stop t += n;                                                               \
  totalt += n;
...
getq ()
{
  ...
  if (a < 0 || a > oldp)
    stop /*bad old*/
        n
        = old[a].size; /*push 1 frozen, maybe to edit!*/
...
deln ()
{
  ...
  if (m < 0 || n < 0)
    stop /*delete n after stack base + m*/
        m
        += adp + base; /*m + address of base in stack!*/
...
#define enterq(A)                                                              \
  ...
  enter (A) oldp++;                                                            \
  ndecl++;                                                                     \
  alast = old[oldp].start = Q[Qnum (#A)].start = qp + 1;                       \
  writeq (A##text);                                                            \
...
{
  nQs = 0;
  enter (onetoD) enter (twotoD) enter (mvdsk) enter (x23) enter (x34) enter (
      bsf) enter (oldSQ) enter (add) enter (mul) enter (powr) enter (sub)
      enter (divide) enter (inc) enter (dec) enter (by2) enter (getq) enter (
          insq) enter (findb) enter (incSQ) enter (decSQ) enter (pupat)
          enter (setpat) enter (insn) enter (mvn) enter (deln) enter (intpf)
              enter (def) enter (topf) enter (dof) enter (oldf) enter (bsjmp)
                  enter (ret) enter (rt0) enter (neg) enter (eq) enter (grt)
                      enter (clear) enter (del) enter (up) enter (ex)
                          enter (jmp1) enter (outn) enter (inn) enter (cpn)
                              enter (xmn) enter (outb) enter (inb) enter (cpnb)
                                  enter (xmnb) enter (ip2ds) enter (pip)
                                      enter (pushdp) enter (popdp) enter (toD)
                                          enter (fromD) enter (delD) enter (tsk)
...
decl (0, 1, c999, c4 c5 powr c5 c5 mul sub ret)
    decl (2, 1, testexp, c4 mul dec c3 c2 mul mul mul up mul ret)
        decl (1, 1, fak, up c1 ex rt0 del up dec fak mul ret)
            decl (1, 1, fak2,
                ...
                decl (-1, -1, defnp,
                      c0 toD pushdp dec toD qot def up rt0 dec intpf cpn qot
                          ret) /*num pars on Dstack, quote begin of defproc*/
    ...
    decl (-1, -1, tailrec,
          qot c1 c1 def up qot c2 outb qot ex rt0 del up dec topf dof qot c3
              outb qot ret qot c1 outb c3 bsjmp) /*no bsf:rt0*/
    ...
    void initdecl ()
{
  ...
  alast = 0;
  ndecl = 0;
  enterq (fak) enterq (fak2) enterq (c999) enterq (testexp) enterq (defnp)
      enterq (calltp) enterq (endnp) afrozen
      = qp;
}
...
printQ (long i)
{
  printf ("[%ld:%ld ", i, q[i].Q);
  printf ("%s", Q[q[i].Q].name);
  if (q[i].p < 1.0)
    printf (" %4.3f", q[i].p);
  if (q[i].P < 1.0)
    printf (" %4.7f", q[i].P);
...
printstack ()
{
  ...
  printf ("\n\nSTACK> ");
  for (i = 1; i <= Sp; i++)
    printf ("(%ld:%ld %ld)  ", S[i].task, S[i].cell, S[i].cont);
}
void
printchains ()
{
...
void test1 open (testexp) push (10);
push (5);
