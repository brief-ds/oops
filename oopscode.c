...
typedef short        BOOL        ;   typedef void   (*       FN)  ();
typedef void        (* FN1) (long);  typedef BOOL   (*       BOFN)();
struct  INFOQ       {FN      fn  ;  /*fn is pointing to C-function!*/
		     long    start; /*start position of declared Q */
                     char  * name;};/*we can declare named programs*/
struct  CHAINS      {long    next;  /*all (un)solved tasks in lists*/
                     long    prev;};/*with pointers: back and forth*/
struct  INFOOLD     {long    size;  /*size of frozen previous code */
		     long    start;};/*start address of the program*/
struct  CHANGE      {long    cell;  /*a tape address - got modified*/
                     long    cont;  /*old content is to be restored*/
		     long    task;};/*task number of modified tape */
struct  OP          {long    Q   ;  /*an instruction number or Qnum*/
		     ...
		     double  P   ;};/*probability of Qs before my Q*/
...
void pushcell       (long cell)
    {if  (Sp     >=  stacksize)     {printf("\nStack overflow");stop}
     if  (mark[task][cell])          return;  /*cell was no virgin!*/
     Sp++;    /*else push virgin!*/  S[Sp].cell    = cell;
     S[Sp].task   =  task;           S[Sp].cont    = tape[task][cell];
     mark[task][cell]             =  TRUE;}   /*mark it as modified*/

void popcells (long  ptr){ /*restore all those tapes down to my ptr*/
     while(Sp >ptr) {tape[S[Sp].task][S[Sp].cell]  = S[Sp].cont;Sp--;}}
void unmark   (long  ptr){long  pp = Sp;/*zero marks down to my ptr*/
     while(pp >ptr) {mark[S[pp].task][S[pp].cell]  = FALSE;     pp--;}}
...
long c (long a)                     {return          tape[task][-a];}
long z (long a){ if (a   <=  0)      return          tape[task][-a];
                    /*then a in s[]; else a in q[]*/ return  q[a].Q;}
void set    (long a, long    val){  /*sets non-q[]-a below address1*/
     a   =  -a;                      if (tape[task] [a]==val) return;
     if             (pushenabled)    pushcell(a);   /*only changes!*/
     tape   [task]  [a] =    val;}  /*no checks of a and val, trust*/
void cpabn          (long a, long b, long n,    long min, long  max){
     if(n<0 || bad(a)|| bad (a  + n) ||   b< min  || b+n> max)  stop
     if     (a==b)   return;         tncheck
     if     (b <a)   while  (n  > 0){set (b, z (a)); n--; b++;  a++;}
     else   {a+=n-1; b+=n-1;
                     while  (n  > 0){set (b, z (a)); n--; b--;  a--;}}}
...
void push   (long x){long y= dp+1;   if  (y> maxdp|| minint> x ||
     maxint <x) stop set    (adp,y); set    (adp+y,  x);}
long pop () {long    tmp =   dp;     if     (tmp==0) stop0
     set    (adp,    tmp -   1);     return  c(adp + tmp);          }
long top () {long x= dp;     if (x== 0)stop0 return  c(adp + x);    }
long ds     (long n){if     (n < 0|| n  >    maxdp)  stop0
                     return  c(adp + n);}           /*nth in stack!*/
void down   (long n, long a){long i= dp-n+1;    /*cp n down, onto a*/
     if     (i==  a) return;        /*already done!*/
     if     (a  > i) stop    set    (adp,    a+n);  /*make a new dp*/
...
void pushD  (long x){if(Dp== maxdp|| x < minint ||   maxint< x) stop
     set    (aDp,    Dp+ 1);         set    (aDp  +  Dp,     x);    }
long popD() {long    tmp =   Dp;     if     (tmp==0) stop0
     set    (aDp,    tmp -   1);     return  c(aDp+  tmp);          }
long topD() {long x= Dp;     if (x== 0)stop0 return  c(aDp + x);    }

long C      (long a){return  tape   [task][- acp -a- cp];           }
void setC   (long a, long    val)   /*set local vars in a top frame*/
    {if     (maxint< val  || val   < minint) stop   /*out of bounds*/
     set    (acp  +  cp+a,   val);}     /*address of param on stack*/

void pushblock      (long m, long n){long i= cp,     a=base,/*C(2)!*/
     j    =  dp;     if  (m> j) stop i+=3;   if(i+3> maxcp)     stop
    ...
    /*new is oldbase:ALL are params! Or base below m params @ stack*/
     ...
     jumped= TRUE;}                 /*jumped as each call varies ip*/
void calla  (long m, long n, long a){pushblock (m,n);setC(1,a);     }
                    /*ip= a; m in; n out;m<0:all in; if n<0 ret all*/
void ret () {long i= cp,     n= out; if (i<1)stop   /*pop? no frame*/
     if     (n >dp)  stop   /*cannot restore as many as I requested*/
     if     (n >=0)  down(n, base); /*return values: replace inputs*/
     set    (acp,    i-3);}         /*resets cp, ip; onestep: ip++!*/
...
void and    () {if  (pop()&& pop())  push(1);        else push(0);  }
void or     () {if  (pop()|| pop())  push(1);        else push(0);  }
void not    () {if  (pop()<= 0)      push(1);        else push(0);  }
void eq     () {if  (pop()== pop())  push(1);        else push(0);  }
void geq    () {if  (pop()>= pop())  push(1);        else push(0);  }
void grt    () {if  (pop()>  pop())  push(1);        else push(0);  }
void c0     () {push(0);          }  void    half() {push(pop()/2); }
void inc    () {push(pop() + 1);  }  void    dec()  {push(pop()-1); }
void add    () {push(pop() + pop());}
void sub    () {long x     = pop();  push(pop()-x);}/*jue don't -()*/
void mul    () {push(pop() * pop());}void    by2()  {push(pop()*2); }
void divide () {long x     = pop();  if(x == 0) stop push(pop()/x); }
void remnant() {long x     = pop();  if(x == 0) stop push(pop()%x); }
void neg    () {long x     = pop();  if(x==0)push(1);else push(-x); }
void powr   () {long n     = pop(),i,m=pop();/*m>=0, to n-th power */
     if        (m <= 1)     {push(1);return;}i=m;   /*instantaneous*/
     while  (n> 1 && minint< i && i< maxint){i*=m;   n--;}  push(i);}
void up     () {push(top());}
void del    () {long x     = dp;     if(x==0)stop    set(adp,x-1);  }
void clear  () {set (adp,    0);}                   /*delete stack!*/
void ex     () {long x,      m  =    dp;             if (m < 2) stop
	        x=ds(m); set(adp+m,  ds(m-1));       set(adp+m-1,x);}
void xmn    () {long x,   m= pop(),  n=pop();/*exchg stack elements*/
     m    =     dp-m+1;   n= dp -    n+1;    /*above top entry, too*/
     if     (m  < 1  ||   m> maxdp|| n<1 ||  n>maxdp)stop
     x    =  ds  (m);        set    (adp+m,  ds(n)); set(adp+n,  x);}
void outn   () {long n     = pop();  push(ds(dp-n+1));}     /*up n!*/
void inn    () {long n     = pop();  n =     dp-n+1;        /*down!*/
                if(n<1 || n> maxdp)  stop    set    (adp+ n, top());}
void cpn    () {long i,j, n= pop();  if(n<0) stop    i =  dp;
     j=i+n; if (j>maxdp)stop set    (adp,j); j+=adp; i+=adp;/*true!*/
     tncheck   /*copy*/while(n > 0) {set    (j,c(i));n--; i--; j--;}}

void xmnb   () {long x,   m= pop(),  n=pop();/*exchg stack elements*/
     m    =     base+m;   n= base+n;/*index relative to base ptr C2*/
     if     (m  < 1  ||   m> maxdp|| n<1 ||  n>maxdp)stop
     x    =  ds  (m);        set    (adp+m,  ds(n)); set(adp+n,  x);}
void outb   () {long n     = pop();  push(ds(base+n));}/*nth + base*/
void inb    () {long n     = pop();  n =     base+n;/*top to base+n*/
                if(n<1 || n> maxdp)  stop    set    (adp+ n, top());}
void cpnb   () {long n     = pop(); /*copy n on base onto my stack */
...
void popdp  () {long a=pop();if(a<0 ||maxdp <a) stop set(adp,a);    }
...
void qot    () {set (aquoteflag,1);}/*and next quote will reset it!*/

void toD    () {pushD(pop());}       void fromD  () {push (topD()); }
void delD   () {long x     = Dp;     if(x==0)stop    set(aDp,x-1);  }
...
void exec   () {long n=pop();if (n<1 || n> nQs) stop (* Q[n].fn)(); }
                    /*views n as the name of a token and executes! */
void def () {long i= 1 +     fnp;    if (i > maxfn)  stop  /*new fn*/
     set    (afnp,i);i = 3*i+afnp;  /*make a name:it is last name+1*/
     set(i,  ip+1);  set(i-1,pop()); set(i-2,pop());}/*selfmade fn!*/
    /*label= ip+1 ;  num ret values; num parameters; recursion ok! */
void topf()         {push(   fnp);}   /*push the top selfdefined fn*/
void popf()         {long i= fnp;    if     (i == 0) stop    push(i);
     set (afnp,i-1);}                    /*undefines last function!*/
void dof ()         {long n= pop(); /*calls selfmade fn by its name*/
     if  (n< 1   ||  n>fnp)  stop                   /*no such name!*/
     n =  afnp+3*n;  pushblock      (c(n-2),c(n-1));/*nums ins/outs*/
     setC(1, c(n));}                /*ip= address of a called label*/
void intpf()        {long n= fnp;   /* get number of inputs of topf*/
     if(n<1) stop  /*no such name!*/ push(c(afnp+3*n-2));}/*num ins*/
void outpf()        {long n= fnp;   /* get number of ouputs of topf*/
     if(n<1) stop  /*no such name!*/ push(c(afnp+3*n-1));}/*num ins*/
void oldf()         {long a= pop();  if(a<0||a>oldp) stop /*bad old*/
     pushblock(0,0); setC(1, old[a]. start);} /*last in last out ok*/
                                    /*juergen better pushbl(-1,-1)?*/
void rt1()          {if  (0< pop())  ret();}
void rt0()          {if  (0>=pop())  ret();}        /*cond. returns*/
void nop()          {;}             /*to structure a program "("")"*/

void stf()  {setC   (1,pop()+adp);} /*sets ip to run code on stack!*/
void basef(){setC   (1,base+ adp);} /*ip = base, run code on stack!*/
void bsjmp(){setC   (1,base+ adp   + pop());} /*ip = base + n on ds*/
void bsf()  {calla  (-1,-1,base+adp+ pop()+1);}/*ret later to code!*/

/*** *************** SOME PRIMITIVES TO EDIT CODE ON THE DATA STACK*/
void getq()         {long a= pop(),n;if(a<0||a>oldp) stop /*bad old*/
     n =     old[a]. size;          /*push 1 frozen, maybe to edit!*/
     cpabn  (old[a]. start,  adp+1+dp,     n, adp+1, aendstack);
     set    (adp,    dp+n);}
void insq()         {long b= pop(),  n, a  = pop(),  m=dp-base-b;
     if     (a<0  || a>oldp  || b<0  || m<0) stop   /*insert frozen*/
     n=old[a].size;  b+=adp+ base+1;/*right above b+ top stack base*/
     cpabn  (b,              b+n,    m,              adp+1,aendstack);
     cpabn  (old[a]. start,  b,      n,              adp+1,aendstack);
     set    (adp,    dp+n);}        /*after base + b insert the q^a*/
void find()         {long k= pop(),  i=dp;     /*get position of k!*/
     while  (i > 0   && c(i+ adp)!=k && t< T*P){i--; inct}   push(i);}
void findb  ()      {long k= pop(),  i=base+1;/*find pos above base*/
     while          (c(adp + i) !=k  && t<T* P){i++; inct}   push(i);}

void deln()         {long n= pop(),  m=pop();
     if     (m<0  || n  < 0) stop   /*delete n after stack base + m*/
     m  +=   adp +   base;          /*m + address of base in stack!*/
     cpabn  (m+n+1,  m+1,            dp-base-n,      adp+1,aendstack);
     set    (adp,    dp-n);}
void mvn()          {long b= pop(),  n=pop(),a=pop(),m=base+ adp;
     cpabn          (m+a,    m+b,    n,      adp+1,  aendstack);
     a     = b+n-1;  if  (a> dp)     set(adp,a); }
                    /*copy n from a  on base C2 to b maybe reset dp*/
void insn()         {long b= pop(),  n=pop(),a=pop(),m=dp- base-b;
     if     (b<0  || m<0  || n<1)    stop b+=adp+base+1;a+=adp+base;
     cpabn          (b,      b+n,    m,              adp+1,aendstack);
     cpabn          (a,      b,      n,              adp+1,aendstack);
     set    (adp,    dp+n);}        /*insert n: from base+a after+b*/
...
long                 dsize,          envsize;
void initenvpars()  {dsize=maxtask+1;envsize      =  3 * dsize;}
long gettop (long    tower)         {return       c (hanoff)        ;}
long getdisk(long    tower,  long    ptr) {return c (hanoff+ptr)    ;}
void settop (long    tower,  long    val)      {set (hanoff,val)    ;}
void setdisk(long    tower,long ptr, long val) {set (hanoff+ptr,val);}
void inittaskhanoi  (long    n)     {long i;         task=n;
     set(atask, n);  settop (1,n);   settop (2,0);   settop (3,0);
     for(i=1;i<=n;   i++)           {setdisk(1,i,    n+1-i);
                                     setdisk(2,i,0); setdisk(3,i,0);}
     clear();push(1);push(2);push(3);push(n);}
void movdisk(long a, long b){long    topa=gettop(a), topb;
     if (topa ==0)   stop            topb=gettop(b);
     if (topb > 0 && getdisk(b,topb)<getdisk(a,topa))stop
     topb++;         settop (b,topb);setdisk(b,topb, getdisk(a,topa));
     setdisk(a, topa,0);             settop (a,topa- 1);            }
void printhanoi()   {long tower,tmp, i; printf      ("\n\nENV:  ");
     for(tower= 1;   tower<= 3;      tower++){
         tmp  =      gettop (tower); printf("[");
         for(i= 1;   i< tmp; i++)    printf("%ld,",   getdisk(tower,i));
         printf     ("%ld",   getdisk(tower,  i));    printf("]  ");}}
BOOL successhanoi() {return(c(aenv)==0 &&  c(aenv +  dsize)==0);}
                                                    /*pegs 1&2 empty*/
void tsk()          {push(c(atask));}  /* primitive: get task size;*/
void mvdsk()        {long            a = ds(base+1), b = ds(base+3);
     if (a<1 || a>3 || b<1 || b>3 || a== b)     stop movdisk(a,b);  }
void x23()          {push(2);        push(3);xmn();}/*xch mvdsk par*/
void x34()          {push(3);        push(4);xmn();}

/*** *************** *************** PROBLEMSPECIFIC SYMMETRY ON D */
/*** goal:  given n, push n 1s & 2s, symmetrically @ second D stack*/
void onetoD   ()    {pushD(1);}      void twotoD () {pushD(2);}
void inittasksymm   (long n){task=n; set(atask, n);  clear();push(n);}
BOOL successsymm()  {long i= aDp   + Dp,             j = i-task-task;
     if (j<= aDp)    return  FALSE;  j+=task;  /*jue c(i) too slow:*/
     while  (i>j  && tape[task][-i]  == 2)   i--;   /*success if on*/
     if (i !=  j)    return  FALSE;  j-=task;       /*D: n 1s, n 2s*/
     while  (i>j  && tape[task][-i]  == 1)   i--;   /*assumes it is*/
     if (i== j)      return  TRUE;   return  FALSE;}/*instantaneous*/
...
long Qnum  (char     name[])        {long i=1;      /*name to Qnum!*/
     while (i   <=   nQs     &&      strcmp(name,    Q[i].name)) i++;
     if(i > nQs)    {printf("NEW:"); printf("%s", name);   printf("\n");}
     return i;                                                      }
long SQnum (char     name[])        {long i= 1,  n = Qnum(name);
     while (i   <=   nSQs    &&      n != SQ[i])     i++;
     if(i > nSQs)   {i = 0;  printf("NEW SQ!");}     return i;      }
void writeq         (char    s[])   {char * p      = strtok(s,   " ");
     while          (p!=0)  {qp++;   q[qp].Q       = Qnum(p);
     q[qp].p = 1.0;  q[qp].P = 1.0;  p             = strtok(0," ");}}
    /*eg char foo[]="c3 up mul cp3"; writeq (foo) to write foo>>q[]*/

/*to decide which of your C-programs to use to build complex tokens*/
void initsimple()   {nQs = 0;        enter(onetoD)   enter(twotoD)
...
decl(0,1,c999,       c4 c5 powr c5   c5 mul sub ret)
decl(2,1,testexp,    c4 mul dec c3   c2 mul mul mul  up  mul ret)
decl(1,1,fak,        up c1 ex rt0    del up dec fak  mul ret)
decl(1,1,fak2,       c1 c1 def up c1 ex rt0 del up   dec topf dof mul ret)
decl(-1,-1,defnp,    c0 toD pushdp   dec toD qot def up rt0 dec
     intpf cpn qot   ret)      /*num pars on Dstack, quote begin of defproc*/
...
decl(-1,-1,tailrec,  qot c1 c1 def   up qot c2 outb  qot ex rt0 del
     up dec topf dof qot c3 outb qot ret qot c1 outb c3 bsjmp)/*no bsf:rt0*/

/*to decide which of numerous decl'd programs really go to q[]stack*/
void initdecl ()    {qp=0;  oldp=0;  alast=0;ndecl=0;
...
void initSQ()       {nSQs  = 0;     /*basic programs for the search*/
     ENTER(c2)       ENTER(c3)       ENTER(mul)      ENTER(getq)
     ENTER(bsjmp)}
void initSQall()    {long i; for(i=1;i<=nQs; i++) SQ [i]=i;nSQs=nQs;}

/*** *************** *************** ***** REMAINING INITIALIZATION*/
void initaddresses(){               /*first: initSQ, & initenvpars!*/
     acp         =   amin;           aendcalls     = acp  +    maxcp;
     adp         =   aendcalls + 1;  aendstack     = adp  +    maxdp;
     aDp         =   aendstack + 1;  aendDs        = aDp  +    maxdp;
     afnp        =   aendDs    + 1;  aendfns       = afnp + 3* maxfn;
     avp         =   aendfns   + 1;  aendvs        = avp  + 2* maxv ;
     aendvecs    =   aendvs    +     maxdata;atask = aendvecs    + 1;
     apatp       =   atask     + 1;  acurp         = apatp       + 1;
     aendpats    =   acurp+(maxpat+1)*(nSQs + 2);   /*2 for sum&max*/
     aquoteflag  =   aendpats  + 1;  aenv          = aquoteflag  + 1;
     awork       =   aenv + envsize; aendwork      = 0;}
...
void initmaxent()   {long i;
     set(apatp, 0);  set(acurp,0);  /*0=only initial search pattern*/
     setsum(nSQs);   setmax(1);     /*initialize all by max entropy*/
     for(i = 1;      i<= nSQs; i++)  setp(i,  1);}
void probsymm       (long n)        {task=n;         initmaxent();
     boost(c1)       boost(c2)       boost(dec)      boost(by2)
     boost(oldSQ)}
void probhanoi      (long n)        {task=n;         initmaxent();
     boost(c3)       boost(c4)       boost(c5)       boost(dec)
     boost(by2)      boost(oldSQ) }
void inittapes()    {long j;
     for(task=1;     task<=maxtask;  task++) {
                     for(j = 0;      j<= tapesize;   j++)
                        {mark[task] [j]= FALSE; tape[task][j]=0;} ;}}
void inittasks()    {long i;        /*get open chain with all tasks*/
     anchor      =   0;             /*starts list of solved tasks  */
     for(i=1;i  <=   maxtask;  i++)
                    {list[i].next=   i- 1;   list[i].prev=i+ 1;
                    (*inittaskfn)(i);(*probfn)(i);   printhanoi();}}
void initbasics(){   pushenabled  =  FALSE;
     initsimple();   initdecl();     initSQall();    upSQ=nSQs;
     initenvpars();  initaddresses();inittapes();    inittasks();
     q[afrozen].Sp=0;Sp=0; totalt=0; alast=afrozen+1;
     pushenabled   = TRUE;}

/*** *************** *************** *************** OUTPUT ********/
     #define  pr(A)  printf("\n");   printf(#A);     printf(":%ld",A);
     #define  prlolo(A)  printf(#A); printf(":"); printlolo(A);printf("\t");
     #define  prc(A) printf("\n\n"); printf(#A);     printf(":%ld ",c(a##A));
void printlolo(lolo  k)             {long i,j; /*for long long ints*/
     if     (k <=    1000000000)    {i = k;          printf("%ld",i);}
     else   {j  = k/ 1000000000;
             i  = k% 1000000000;     printf("%ld%09ld",j, i);}        }
void printmarks()   {long i;         printf         ("\nMARKS:");
     for(i = 0; i <= tapesize; i++)  printf("%d ",   mark[task][i]);}
void printQ (long i){printf("[%ld:%ld ",i, q[i].Q);
     printf("%s", Q[q[i]. Q].name); if(q[ i].p<1.0)printf(" %4.3f",q[i].p);
     if(q[i].P<1.0)  printf(" %4.7f",q[i].P);        printf("]");}
void printold(long n){long i,j;      printf("\n%ld: ",n);
     j=old[n].start+ old[n].size-1;
     for  (i= old[n].start;i<=j;i++) printQ(i);      printf("\n\n");}
void printolds()    {long i; printf          ("\nOLD PROGRAMS:\n");
     for(i=1;i<=oldp;i++)printold(i);printf  ("\nNOT OLD YET:\n");
     for(i=old[oldp].start+old[oldp].size;i<=qp;i++) printQ(i);}
void printq(){long i;printf("\nq: ");for(i=1; i<=qp; i++)printQ(i);}
void printSQ()      {long i;         printf         ("\n\nSEARCHQS:");
     for(i = 1; i <= nSQs;   i++)   {printf("[%ld:%ld ",i, SQ[i]);
     printf("%s", Q[SQ[i]].name);  printf         ("]  ");}       }
void printblock     (long a){long k; printf("%ld:",   a-acp);
     printf("[ip %ld ",c(++a));       printf         ("dp %ld ",c(++a));
     printf("out %ld",c(++a));        printf("]  ");                }
void printpat       (long n){long i, a = acurp + n *(nSQs + 2);
     printf("\n%ld:", n);             printf("[%ld ",  c(a+1));
     printf( "max %ld ",c(a+2));      printf("probs: ");
     for(i=1;nSQs>=i;i++)    printf("%ld ",c(a+2+i)); printf("]  ");}
void printtape()    {long i;         printf   ("\n\n-TAPE:");prc(cp)
     printf("calls:");for(i=acp;i  < acp+maxcp; i+=3)printblock(i);
     prc(dp)         printf         ("data stack:");
     for(i=adp+1;i<= aendstack; i++) printf("%ld:%ld ",i-adp,c(i));
     prc(Dp)         printf         ("2nd data stack:");
     for(i=aDp+1;i<= aendDs;    i++) printf("%ld:%ld ",i-aDp,c(i));
     prc(fnp) printf("fns:");   for (i=afnp+1;   i<= aendfns;  i+=3)
     printf    ("%ld:(%ld,%ld,%ld) ",   (i-afnp)/3 + 1,  c(i),c(i+1),c(i+2));
     prc(vp)         printf("vs:");  for(i=avp+1;    i<=aendvs;  i+=2)
                     printf         ("(%ld,%ld) ",     c(i), c(i+1));
     printf("data:");for(i=aendvs+1; i<=aendvecs;i++)printf("%ld ",c(i));
     prc(task)   prc(patp) prc(curp) for(i=0;maxpat>=i; i++) printpat(i);
     prc(quoteflag)
     printf("\n\nenv: ");for(i=aenv; i<awork;    i++)printf("%ld ",c(i));
     printf("\n\nwork:");for(i=awork;i<=aendwork;i++)printf("%ld ",c(i));}
void printstack()   {long    i;      printf         ("\n\nSTACK> ");
     for(i=1; i<=Sp; i++)            printf         ("(%ld:%ld %ld)  ",
                                     S[i].task, S[i].cell,S[i].cont);}
void printchains()  {long i, tmp;    printf         ("\n\nSOLVED:");
     i=anchor;       while  (i!=0)  {printf("%ld,",i);i=list[i].prev;}
     printf         ("UNSOLVED:" );  if (! unsolved) return;
     i = tmp =  list[task].next; do {printf("%ld,",i);i=list[i].next;}
    /*tmp ok even if task solved*/   while          (i != tmp);     }
void show(){printf("\n\n------------ SHOW START ----------------\n");
     printolds();    printSQ();      printhanoi();   printtape();
     printchains();  printstack();   printf("\n\n"); prlolo(T);
     prlolo(prefs);  prlolo(insts);  prlolo(totalt); prlolo(topt);
     pr(topSp);      pr(afrozen);    pr(alast);      pr(acurrent);
            printf("\n\n------------ SHOW END ------------------\n");}
...
void removetask()   {unsolved--;    /*remove current task from list*/
     list[list[task].prev].next   =  list[task].next;
     list[list[task].next].prev   =  list[task].prev;
     list[task].prev              =  anchor;
     anchor                       =  task;}   /*task.next unchanged*/
void reinserttask(){ unsolved++;    /*anchor!=0;undo recent removal*/
     task                         =  anchor;  /*task.next unchanged*/
     anchor                       =  list[task].prev;
     list[task].prev              =  list[list[task].next].prev;
     list[list[task].next].prev   =  task;
     list[list[task].prev].next   =  task;}/*ok when unsolved empty*/
void dummy(){;}
void onestep()      {long n=z(ip);  /*execute single Q--instruction*/
     ...
     if (!   jumped) setC(1, ip+1);                 /*increments ip*/
     if (   (*       successfn)())   removetask();  /*jue: if halt?*/
     task =  list   [task].  next;} /*ok - even when task removed!!*/
BOOL ipok() {long a= ip;                            /*is ip  valid?*/
     if     (a     < amin || a    >  qp)             return  FALSE;
     if     (z(a)  < 1    || z(a) >  nQs   )         return  FALSE;
                                                     return  TRUE ; }
void stepuntil(){   /*til all solved or halt or time out or search!*/
     halt = FALSE;
...
void maybepushQ(){  /*exclusively at peculiar search address qp + 1*/
     if (halt    ||  ip != qp+1)     return;        /*wrong address*/
     if (T*P*pmax()<(t + unsolved) * psum()) return;/*lack of time!*/
     if (qp       >= amax)
                    {printf("\n CODE CRASH");return;}
...
void restoretasks(long below){while (anchor != below)reinserttask();}
void restoreQ(){             /*reset all like before use of top's Q*/
...
BOOL nextQ() {long   i=q[qp].SQ,     sum;            double tmp;
     if (i!=0)       restoreQ();    /*if this SQ not freshly pushed*/
     if (i==nSQs)    return(FALSE); /*if last search Q, no next SQ!*/
     sum  = psum();  tmp=T*P/ (sum* (t+unsolved));  /*restored prob*/
     do{i++;} while (i<=nSQs   &&    tmp*pn(i)< 1.0);/*enough time?*/
     if(i> nSQs)     return(FALSE); /*no alternative left for my qp*/
    /*else:*/        q[qp].SQ  =  i;/*next SQ is ok, sufficient prb*/
                     q[qp].Q   =     SQ[i];         /*Qnum from SQ!*/
                     q[qp].p   =    (double) pn(i) / sum;
     P             = q[qp].P   *     q[qp].p;
     prefs++;        return(TRUE);} /*divide prob by the normalizer*/
void backtrack()    {while (qp   >   afrozen &&   (! nextQ())) qp--;}

/*** *************** *************** PROBLEM SOLVING AND SEARCHING */
void search(){      /*simultaneously solve all tasks in a task ring*/
     t=0;            P = 1.0;       /*now do forever (until exitus)*/
     for   ( ; ; )  {stepuntil();   /*done?  new Q?  halt?   t/P>T?*/
                     if(unsolved==0) {done   =  TRUE;return;}/*done*/
                     maybepushQ();  /*push, if new Q request & time*/
                     backtrack();   /*next Q chosen, or no SQs left*/
                     if(qp==afrozen) return;}} /*was not successful*/

void reset(){/*reset all tapes/marks for all tasks - but leave q[]!*/
     unmark(0);      popcells(0);    restoretasks(0);               }
void searchring     (long n,  long m){              /*task m>task n*/
     unsolved      = m  - n + 1;    /*num of chain's unsolved tasks*/
     if (unsolved  > T)   return;   /*lacks the time to initialize!*/
     list[m].prev  = n;             /*close a chain: m, m-1, ..., n*/
     list[n].next  = m;
     pushenabled   = FALSE;         /*set initial ip's, do not push*/
     for    (task  = n;  task <=  m; task++) setC(1, acurrent);
     pushenabled   = TRUE;  /*1st ip of main call is at my acurrent*/
     task          = m;             /*start with top task in a ring*/
     search();
     if (unsolved == 0){printf("\n\n ALL SOLVED! "); show();}
     reset();       /*totally resets tapes & marks & unsolved tasks*/
     list[n].next  = n - 1;                         /*reopens ring!*/
     list[m].prev  = m + 1;                                         }

void solveupto(long  n){/*(tasks,ip)=(n, alast), or (1-n,afrozen+1)*/
     qp = afrozen;   T = 1;          done  = FALSE; /*forever loop:*/
     for(;;)        {printf("\n");   prlolo(T);
                     prlolo(insts);  prlolo(prefs);  prlolo(totalt);
                     acurrent      = alast; /*known: solves tasks<n*/
                     searchring     (n,      n);    /*solve only n!*/
                     if     (done)   return;        /*exit for-loop*/
	             acurrent      = afrozen+1;     /*1st un-frozen*/
	             searchring     (1,      n);    /*each up to n!*/
                     if     (done)   return;        /*exit for-loop*/
		     if     (T    >= maxlolo / 2)   {printf
		                    ("\n\nT: CRASH");return;}
                     T *= 2;}}
                    /*jue redundant: 1st alast above afrozen anyway*/

void freeze(){      /*from acurrent: 1 - n solvable; keep newer Q's*/
     afrozen       = qp; /*maybe new top frozen addr,alast>0 surely*/
     if (alast    != acurrent)      {oldp++;
         old[oldp].  start=alast;    old[oldp].size= acurrent-alast;}
     alast         = acurrent;                      /*is always ok!*/
     q[afrozen].Sp = 0;} /*unmark in next 1st pushQ: goes down here*/

void run()  {long i;
     for(i=1;        maxtask>=i;i++){solveupto(i);   freeze();}}
void test()         {inittaskfn =   (*inittasksymm);
     successfn    = (*successsymm);  probfn       = (*probsymm);
     initbasics();   run();
     inittaskfn   = (*inittaskhanoi);successfn    = (*successhanoi);
     probfn =       (*probhanoi);
     pushenabled  =  FALSE;          inittasks();    pushenabled=TRUE;
     run();}
...
void test1   open   (testexp)        push(10);       push(5);close
void test2   open   (fak2)   clear();push(12);       close
int  main()         {test();}
