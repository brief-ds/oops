...
void printhanoi()   {long tower,tmp, i; printf      ("\n\nENV:  ");
     for(tower= 1;   tower<= 3;      tower++){
         ...
         for(i= 1;   i< tmp; i++)    printf("%d,",   getdisk(tower,i));
         printf     ("%d",   getdisk(tower,  i));    printf("]  ");}}
...
long Qnum  (char     name[])        {long i=1;      /*name to Qnum!*/
     ...
     if(i > nQs)    {printf("NEW:"); printf(name);   printf("\n");}      
     return i;                                                      }   
...
void printlolo(lolo  k)             {long i,j; /*for long long ints*/
     if     (k <=    1000000000)    {i = k;          printf("%d",i);}
     else   {j  = k/ 1000000000;
             i  = k% 1000000000;     printf("%d%09d",j, i);}        }
void printmarks()   {long i;         printf         ("\nMARKS:");
     for(i = 0; i <= tapesize; i++)  printf("%d ",   mark[task][i]);}
void printQ (long i){printf("[%d:%d ",i, q[i].Q); 
     printf( Q[q[i]. Q].name); if(q[ i].p<1.0)printf(" %4.3f",q[i].p);
...
void printSQ()      {long i;         printf         ("\n\nSEARCHQS:"); 
     for(i = 1; i <= nSQs;   i++)   {printf("[%d:%d ",i, SQ[i]);
     printf(Q[SQ[i]].name);  printf         ("]  ");}       }
void printblock     (long a){long k; printf("%d:",   a-acp);  
     printf("[ip %d ",c(++a));       printf         ("dp %d ",c(++a));   
     printf("out %d",c(++a));        printf("]  ");                }
void printpat       (long n){long i, a = acurp + n *(nSQs + 2); 
     printf("\n%d:", n);             printf("[%d ",  c(a+1)); 
     printf( "max %d ",c(a+2));      printf("probs: ");  
     for(i=1;nSQs>=i;i++)    printf("%d ",c(a+2+i)); printf("]  ");}
void printtape()    {long i;         printf   ("\n\n-TAPE:");prc(cp) 
     ...
     printf    ("%d:(%d,%d,%d) ",   (i-afnp)/3 + 1,  c(i),c(i+1),c(i+2));
     prc(vp)         printf("vs:");  for(i=avp+1;    i<=aendvs;  i+=2)
                     printf         ("(%d,%d) ",     c(i), c(i+1));
     printf("data:");for(i=aendvs+1; i<=aendvecs;i++)printf("%d ",c(i));
     prc(task)   prc(patp) prc(curp) for(i=0;maxpat>=i; i++) printpat(i);
     prc(quoteflag)
     printf("\n\nenv: ");for(i=aenv; i<awork;    i++)printf("%d ",c(i));
     printf("\n\nwork:");for(i=awork;i<=aendwork;i++)printf("%d ",c(i));}
void printstack()   {long    i;      printf         ("\n\nSTACK> ");
     for(i=1; i<=Sp; i++)            printf         ("(%d:%d %d)  ", 
                                     S[i].task, S[i].cell,S[i].cont);}
void printchains()  {long i, tmp;    printf         ("\n\nSOLVED:"); 
     i=anchor;       while  (i!=0)  {printf("%d,",i);i=list[i].prev;}
     printf         ("UNSOLVED:" );  if (! unsolved) return; 
     i = tmp =  list[task].next; do {printf("%d,",i);i=list[i].next;} 
